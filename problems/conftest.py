...
def pytest_terminal_summary(terminalreporter):  # type: ignore[no-untyped-def]
    ...
    # Auto-update stats.json for problems whose default cases all passed.
    try:
        rootdir = Path(terminalreporter.config.rootdir)  # type: ignore[arg-type]
        problems_dir = rootdir / "problems"
        # Discover problems that have real TEST_CASES (non-empty and not stub-only)
        implemented: set[str] = set()
        opt_in_optimal: set[str] = set()
        for p in sorted(problems_dir.iterdir()):
            if not p.is_dir():
                continue
            sol_path = p / "solutions.py"
            stats_path = p / "stats.json"
            if not sol_path.exists() or not stats_path.exists():
                continue
            try:
            ...
            if isinstance(raw_cases, list) and raw_cases:
                implemented.add(p.name)
            try:
                if bool(ns.get("TEST_MARK_OPTIMAL_ON_PASS")):
                    opt_in_optimal.add(p.name)
            except Exception:
                pass

        # Collect pass/fail per problem from pytest reports of default-case tests
        def problem_from_nodeid(nodeid: str) -> str | None:
            # Nodeid example: problems/all_problems_spec.py::test_default_cases[0001-two-sum-...]
            if "test_default_cases[" not in nodeid:
                return None
            try:
                param = nodeid.split("test_default_cases[", 1)[1]
                # first param is the problem dir name; split at first '-' of next param safely by closing ']'
                # The param list is hyphen-joined; problem dir contains a dash after number, so take until
                #  next '-' followed by a capitalized sol name or a path.
                # More robust: split by '-' and rejoin first two tokens to get 'NNNN-slug'
                parts = param.split("-")
                if len(parts) >= 2:
        ...
        any_passed: dict[str, bool] = {}
        any_failed: dict[str, bool] = {}
        for rep in list(terminalreporter.stats.get("passed", [])) + list(
            ...
            try:
                ...
                nodeid = ""
            prob = problem_from_nodeid(nodeid)
            if not prob:
                continue
            if prob not in implemented:
                continue
            if rep.when != "call":  # only consider test call outcomes
                continue
            if rep.passed:
                any_passed[prob] = True
            if rep.failed:
                any_failed[prob] = True

        updated: list[str] = []
        for prob in sorted(implemented):
            if not any_passed.get(prob):
                continue  # no default-case tests ran
            if any_failed.get(prob):
                continue  # some default-case failed
            stats_path = problems_dir / prob / "stats.json"
            try:
                data = json.loads(stats_path.read_text(encoding="utf-8"))
            except Exception:
                continue
            # Only set baseline_impl; never downgrade or touch other fields here.
            if not isinstance(data, dict):
                continue
            if data.get("baseline_impl") is not True:
                data["baseline_impl"] = True
            # Optional opt-in: mark optimal_solution when all tests pass and module opted in
            if prob in opt_in_optimal and data.get("optimal_solution") is not True:
                data["optimal_solution"] = True
            try:
        ...
        if updated:
            terminalreporter.write_line("")
            terminalreporter.write_line(f"Updated stats.json fields for: {', '.join(updated)}")
    except Exception:
        # Never let stats update break test reporting
        pass
